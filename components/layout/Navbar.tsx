    ...
    <header className="border-b bg-white sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-6 text-sm font-medium text-gray-600">
          ...
          <Link href="/elections" className="hover:text-gray-900 transition-colors">Elections</Link>
          {(currentUser.role === 'student' || currentUser.role === 'advisor') && (
            <Link href="/dashboard" className="hover:text-gray-900 transition-colors">My Clubs</Link>
          )}
          {currentUser.role === 'admin' && (
            <Link href="/admin" className="hover:text-gray-900 transition-colors">Admin Panel</Link>
          )}
        </nav>
